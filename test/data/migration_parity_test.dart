...
void main() {
  ...
  group('legacy ambiguity is preserved, never guessed', () {
    test(
      'a child with more recorded parents than a family keeps them all',
      () async {
  ...
  group('a migration that cannot complete changes nothing', () {
    test(
      'an unmigratable legacy database keeps its version and its tables',
      () async {
        ...
        try {
          ...
          final tables = probe
              ...
              .toSet();
          final people =
              probe.select('SELECT COUNT(*) AS c FROM persons').first['c']
                  as int;
